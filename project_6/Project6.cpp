...
double sqrtRec(double x, double low_guess, double high_guess) {
    ...
    if (mid*mid == x){
        ...
        return sqrtRec(x, low_guess, mid-(1e-15));
    } else if (mid*mid < x){
        return sqrtRec(x, mid+1e-15, high_guess);
    }
...
int strCompare2(char* str1, char* str2) {
    ...
    if (whatLetter(str1[0]) == -1){
        return strCompare2(str1+1, str2);
    }
    if (whatLetter(str2[0]) == -1){
...
int solveMazeRec(int row, int col){
    ...
    if (row < 0 || col < 0 || row > MATRIX_SIZE-1 || col > MATRIX_SIZE-1) {
        return false;
    }
    if (maze[row][col] >= 1){
        return false;
